...
{{-- Title --}}
@section('html_title', __('ssc/leave_application/detail.title_html'))
@section('page_title', __('ssc/leave_application/detail.title_page'))

@section('body_page_breadcrumb')
    <li class="breadcrumb-item active">{{ __('ssc/leave_application/detail.breadcrumb_level_1') }}</li>
    <li class="breadcrumb-item active"><a href="{{route('ssc.leave_application.index')}}">{{ __('ssc/leave_application/detail.breadcrumb_level_2') }}</a></li>

    @if($mode['isModeCreate'])
        <li class="breadcrumb-item active">{{ __('ssc/leave_application/detail.breadcrumb_create') }}</li>
    @else
        @if($mode['isModeEdit'])
            <li class="breadcrumb-item active">{{ $la->title }}</li>
            <li class="breadcrumb-item active">{{ __('ssc/leave_application/detail.breadcrumb_edit') }}</li>
        @else
            <li class="breadcrumb-item active">{{ $la->title }}</li>
        @endif
    @endif
...
@section('body_main_content')
    ...
    @if($mode['isModeShow'])
        <div class="card">
            <div class="card-body">
                @include('ssc.leave_application.detail.panel')
            </div>
        </div>
    @endif
    ...
    <form id="v_form" enctype="multipart/form-data" class="block form-horizontal form-label-left"
          action="@if(!$mode['isModeEdit']){{route('ssc.leave_application.store')}}@elseif($mode['isModeEdit']){{route('ssc.leave_application.update', $la->uuid)}}@endif"
          method="post">
        @if($mode['isModeEdit'])
            {{method_field('put')}}
        @endif
        <div class="card">
            <div class="card-body">
                ...
                @if(!$mode['isModeCreate'])
                    <x-inputs.text
                        :label="__('ssc/leave_application/detail.lb_applicant_id')"
                        :isReadonly="true"
                ...
                @else
                    ...
                    <x-inputs.select2
                        :label="__('ssc/leave_application/detail.lb_leave_type_id')"
                        :isReadonly="!$mode['isModeCreate']"
                        name="leave_type_id"
        ...
        <div class="card @if($mode['isModeCreate']) d-none @endif" id="application_detail_card">
            <div class="card-body">
                ...
                <x-inputs.text
                    :label="__('ssc/leave_application/detail.lb_title')"
                ...
                {{--                <x-inputs.textarea--}}
                {{--                    :label="__('ssc/leave_application/detail.lb_reason')"--}}
                {{--                    :isReadonly="$mode['isModeShow']"--}}
                {{--                    value="{{$la->reason ?? ''}}"--}}
                {{--                    name="reason"--}}
                {{--                />--}}
                ...
                <x-inputs.file
                    type="multiple"
                    :label="__('ssc/leave_application/detail.lb_appendix')"
                    :isReadonly="$mode['isModeShow']"
                    :isEdit="$mode['isModeEdit']"
                    deleteRoute="ssc.leave_application.file_delete"
                ...
                <x-inputs.date
                    type="datetime"
                    :label="__('ssc/leave_application/detail.lb_start_at')"
                    :isReadonly="$mode['isModeShow']"
                    value="{{$la->start_at ?? ''}}"
                    name="period_start"
                    format="YYYY-MM-DD"
                ...
                <x-inputs.date
                    type="datetime"
                    :label="__('ssc/leave_application/detail.lb_end_at')"
                    :isReadonly="$mode['isModeShow']"
                    value="{{$la->end_at ?? ''}}"
                    name="period_end"
                    format="YYYY-MM-DD"
                ...
                {{--                <x-inputs.text--}}
                {{--                    type="number"--}}
                {{--                    :label="__('ssc/leave_application/detail.lb_duration_day')"--}}
                {{--                    :isReadonly="$mode['isModeShow']"--}}
                ...
                {{--                <x-inputs.text--}}
                {{--                    type="number"--}}
                {{--                    :label="__('ssc/leave_application/detail.lb_duration_hour')"--}}
                {{--                    :isReadonly="$mode['isModeShow']"--}}
                ...
                <x-inputs.select2
                    :label="__('ssc/leave_application/detail.lb_reviewers')"
                    :isReadonly="$mode['isModeShow']"
                    name="reviewers_id[]"
                    value="TODO"
                    multiple="true"
                ...
                @if(!$mode['isModeCreate'])
                    <x-inputs.select2
                        :label="__('ssc/leave_application/detail.lb_status')"
                        :isReadonly="$mode['isModeShow']"
                        name="status"
                        required="true"
                        showCustom="true"
                    >
                        @if($mode['isModeShow'] || Auth::user()->hasRole(config('constants.ROLE.USER')))
                            @switch($la->status)
                                ...
                                <span
                                    class="badge bg-secondary">{{ __('ssc/leave_application/detail.config_constants_LEAVE_APPLICATION_STATUS_PENDING') }}</span>
                                @break
                                @case(config('constants.LEAVE_APPLICATION.STATUS.APPROVE'))
                                <span
                                    class="badge bg-success">{{ __('ssc/leave_application/detail.config_constants_LEAVE_APPLICATION_STATUS_APPROVE') }}</span>
                                @break
                                @case(config('constants.LEAVE_APPLICATION.STATUS.REFUSE'))
                                <span
                                    class="badge bg-dark">{{ __('ssc/leave_application/detail.config_constants_LEAVE_APPLICATION_STATUS_REFUSE') }}</span>
                                @break
                                @case(config('constants.LEAVE_APPLICATION.STATUS.WAIT_ADMIN'))
                                <span
                                    class="badge bg-warning">{{ __('ssc/leave_application/detail.config_constants_LEAVE_APPLICATION_STATUS_WAIT_ADMIN') }}</span>
                                @break
                                @default
                                -
                                @break
                        ...
                        @else
                            @foreach(config('constants.LEAVE_APPLICATION.STATUS') as $key => $value)
                                ...
                                <option
                                    ...
                                    @endif
                                    @endif
                                >{{ __('ssc/leave_application/detail.config_constants_LEAVE_APPLICATION_STATUS_'.$key) }}</option>
                            @endforeach
                        @endif
                    </x-inputs.select2>
                @endif
    ...
    <script>
        $().ready(function () {
            ...
            const getLeaveTypeData = (tid, euuid) => {
                ...
                $.post("{{route('ssc.leave_application.getLeaveTypeData')}}", {tid: tid, euuid: euuid, _token: "{{ csrf_token() }}",},
                    function (data) {
            ...
            $('#id_applicant_id').on('select2:select', function (e) {
