...
{{-- Title --}}
@section('html_title', __('eh/leave_request/detail.title_html'))
@section('page_title', __('eh/leave_request/detail.title_page'))

@section('body_page_breadcrumb')
    <li class="breadcrumb-item active">{{ __('eh/leave_request/detail.breadcrumb_level_1') }}</li>
    <li class="breadcrumb-item active">{{ __('eh/leave_request/detail.breadcrumb_level_2') }}</li>

    @if($mode['isModeCreate'])
        <li class="breadcrumb-item active">{{ __('eh/leave_request/detail.breadcrumb_create') }}</li>
    @else
        @if($mode['isModeEdit'])
            <li class="breadcrumb-item active">{{ $la->title }}</li>
            <li class="breadcrumb-item active">{{ __('eh/leave_request/detail.breadcrumb_edit') }}</li>
        @else
            <li class="breadcrumb-item active">{{ $la->title }}</li>
        @endif
    @endif
...
@section('body_main_content')
    ...
    @if($mode['isModeShow'])
        <div class="card">
            <div class="card-body">
                @include('eh.leave_request.detail.panel')
            </div>
        </div>
    @endif
            ...
            <div class="row">
                ...
                <div class="col-md-3">
                    <x-inputs.text
                        ...
                        :isReadonly="true"
                        name="title_id"
                        value="{{$la->employee->title->title[App::getLocale()]??''}}"
                    />
                </div>
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <x-inputs.text
                        ...
                        :isReadonly="true"
                        value="{{$la->employee->chinese_name ?? ''}}"
                        name="chinese_name"
                    />
                </div>
    ...
    <form id="v_form" enctype="multipart/form-data" class="block form-horizontal form-label-left"
          action="@if($mode['isModeEdit']){{route('eh.leave_request.update', $la->uuid)}}@endif"
          method="post">
        @if($mode['isModeEdit'])
            {{method_field('put')}}
        @endif
        <div class="card">
            <div class="card-body">
                ...
                @endhasanyrole
                    ...
                    <x-inputs.select2
                        :label="__('eh/leave_request/detail.lb_leave_type_id')"
                        :isReadonly="true"
                        name="leave_type_id"
        ...
        <div class="card @if($mode['isModeCreate']) d-none @endif" id="application_detail_card">
            <div class="card-body">
                ...
                <x-inputs.text
                    :label="__('eh/leave_request/detail.lb_title')"
                    :isReadonly="true"
                    value="{{$la->title ?? ''}}"
                    name="title"
                    required="true"
...
{{--                <x-inputs.textarea--}}
{{--                    :label="__('eh/leave_request/detail.lb_reason')"--}}
{{--                    :isReadonly="true"--}}
{{--                    value="{{$la->reason ?? ''}}"--}}
{{--                    name="reason"--}}
{{--                />--}}
                ...
                <x-inputs.file
                    type="multiple"
                    :label="__('eh/leave_request/detail.lb_appendix')"
                    :isReadonly="true"
                    :value="$la->appendix??''"
                    name="appendix[]"
                    fileMax="50000"
                ...
                <x-inputs.date
                    type="datetime"
                    :label="__('eh/leave_request/detail.lb_start_at')"
                    :isReadonly="true"
                    value="{{$la->start_at ?? ''}}"
                    name="start_at"
                ...
                <x-inputs.date
                    type="datetime"
                    :label="__('eh/leave_request/detail.lb_end_at')"
                    :isReadonly="true"
                    value="{{$la->end_at ?? ''}}"
                    name="end_at"
                    format="YYYY-MM-DD HH:mm"
...
{{--                <x-inputs.text--}}
{{--                    type="number"--}}
{{--                    :label="__('eh/leave_request/detail.lb_duration_day')"--}}
{{--                    :isReadonly="true"--}}
...
{{--                />--}}
                ...
                {{--                <x-inputs.text--}}
                {{--                    type="number"--}}
                {{--                    :label="__('eh/leave_request/detail.lb_duration_hour')"--}}
                {{--                    :isReadonly="true"--}}
                ...
                <x-inputs.select2
                    :label="__('eh/leave_request/detail.lb_reviewers')"
                    :isReadonly="true"
                    name="reviewers_id[]"
                    value="TODO"
                    multiple="true"
                ...
                @if(!$mode['isModeCreate'])
                    <x-inputs.select2
                        :label="__('eh/leave_request/detail.lb_status')"
                    ...
                    >
                        @if($mode['isModeShow'])
                            @switch($la->status)
                                ...
                                <span
                                    class="badge bg-secondary">{{ __('eh/leave_request/detail.config_constants_LEAVE_APPLICATION_STATUS_PENDING') }}</span>
                                @break
                                @case(config('constants.LEAVE_APPLICATION.STATUS.APPROVE'))
                                <span
                                    class="badge bg-success">{{ __('eh/leave_request/detail.config_constants_LEAVE_APPLICATION_STATUS_APPROVE') }}</span>
                                @break
                                @case(config('constants.LEAVE_APPLICATION.STATUS.REFUSE'))
                                <span
                                    class="badge bg-dark">{{ __('eh/leave_request/detail.config_constants_LEAVE_APPLICATION_STATUS_REFUSE') }}</span>
                                @break
                                @case(config('constants.LEAVE_APPLICATION.STATUS.WAIT_ADMIN'))
                                <span
                                    class="badge bg-warning">{{ __('eh/leave_request/detail.config_constants_LEAVE_APPLICATION_STATUS_WAIT_ADMIN') }}</span>
                                @break
                                @default
                                -
                                @break
                        ...
                        @else
                            @foreach(config('constants.LEAVE_APPLICATION.STATUS') as $key => $value)
                                ...
                                <option
                                    ...
                                    @endif
                                    @endif
                                >{{ __('eh/leave_request/detail.config_constants_LEAVE_APPLICATION_STATUS_'.$key) }}</option>
                            @endforeach
                        @endif
                    </x-inputs.select2>
                @endif
