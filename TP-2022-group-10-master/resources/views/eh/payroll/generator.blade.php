...
<table class="table">
    ...
    <tbody>
    @foreach($e->leave_application as $lr)
        <tr>
            <td>{{ $lr['period_start'] }}</td>
            <td>{{ $lr['period_end'] }}</td>
            <td>{{ $lr->leaveType->code }}</td>
            <td>{{ $lr['status'] }}</td>
        </tr>
    @endforeach
    </tbody>
...
@php
    //Config Period settings
        //for($j=0; $j<24;$j++){
            $s3_period_start='2021-10-01';
            $s3_period_end='2021-10-31';
...
<table class="table">
    ...
    <tr>
        ...
        <td>
            @php
                $s3_employee_start_year=date("Y", strtotime($e->package[0]['effective_date']));
                $s3_employee_start_month=date("m", strtotime($e->package[0]['effective_date']));
                $s3_employee_start_day=date("d", strtotime($e->package[0]['effective_date']));
                ...
                $s3_mpf_employer_compulsory=$e->package[0]['detail']['mpf_employer_compulsory'];
                $s3_mpf_employer_voluntary=$e->package[0]['detail']['mpf_employer_voluntary'];
                $s3_mpf_employee_compulsory=$e->package[0]['detail']['mpf_employee_compulsory'];
                $s3_mpf_employee_voluntary=$e->package[0]['detail']['mpf_employee_voluntary'];
            @endphp
    ...
    <tr>
        ...
        <td>
            @php
                $s3_basic = $e->package[0]['detail']['basic_salary'];;
                foreach($e->package as $p){
        ...
        <td>
            ...
            @for($i=0; $i<sizeof($selected_array); $i++)
                ...
                @php
                    ...
                    $code = $selected_array[$i]->leaveType->code;
                    ...
                    $is_nwd = $selected_array[$i]->leaveType->is_nwd;
                    $is_adj = $selected_array[$i]->leaveType->is_adj;
                    $is_rse = $selected_array[$i]->leaveType->is_rse;
                    $percentage = $selected_array[$i]->leaveType->percentage;
            ...
            @php
                ...
                foreach($holiday_list as $h){
                    ...
                    if(($s3_period_start_year == $hr_date_year) && ($s3_period_start_month == $hr_date_month)){
                        ...
                        array_push($selected_array, $data);
                    }
                }
            @endphp

            @for($h=0; $h<sizeof($selected_array); $h++)

                @php
                    //print the detail of the selected array, make sure data extracted are correct
                    $type = $selected_array[$h]['holiday']['type'];
                    $period_start = $selected_array[$h]['holiday']['date'];
                    $period_end = $selected_array[$h]['holiday']['date'];
                    $period_days = daysInAPeriod($s3_period_start, $s3_period_end, $period_start, $period_end);
                    $is_nwd = $selected_array[$h]['holiday']->config->is_nwd;
                    $is_adj = $selected_array[$h]['holiday']->config->is_adj;
                    $is_rse = $selected_array[$h]['holiday']->config->is_rse;
                    $percentage = $selected_array[$h]['holiday']->config->percentage;

                    //calculate the non-workday (NWD) deduction
                    if($is_nwd){
                        $is_nwd_count = computeNwd($period_days, $s3_basic, $s3_cal_day);
                        $selected_array[$h]['is_nwd'] = $is_nwd_count;
                    ...
                    if($is_adj){
                        if($is_rse){
                            $is_rse_count = computeAdjRse($period_days, $percentage, $acc_12m_salary, $acc_12m_wd);
                            $selected_array[$h]['is_rse'] = $is_rse_count;
                            $total_holiday += $is_rse_count;
                        }else{
                            $is_adj_count = computeAdj($period_days, $percentage, $s3_basic, $s3_cal_day);
                            $selected_array[$h]['is_adj'] = $is_adj_count;
